...
Similar to the previous workflowr but this time I increase N0 to be 500 and smooth out some of the plot in the result section.
...
We increase the divergence time in every simulation (incremental by 25). 

See next section for the result.

```{r simulate, results='hide', message=FALSE, warning=FALSE}
library(susielove)
library(susieR)
...
Here I smooth out the plot a little to see easier. 
...
w <- 5  # rolling window size (use 3 or 5)
...
df <- data.frame(
  ...
  group_by(group) %>%
  mutate(y_smooth = stats::filter(y, rep(1/w, w), sides = 2) |> as.numeric()) %>%
  ungroup()

ggplot(df, aes(x = x, y = y_smooth, color = group)) +
  geom_line() +
  geom_point() +
  labs(
    x     = "Divergence time",
    y     = "Coverage",
    title = paste0("Coverage as divergence time varies (rolling mean, w=", w, ")"),
    color = "Legend Title"
  )
```

Because I have not ran replications, it is a bit hard to see. But it looks like WS wins overall.

```{r compare mean coverage}
colSums(coverages) / num_reps
```
...
colnames(powers) <- c("In-sample", "Out-of-sample", "Naive shrinkage", "WS", "LW")
...
ggplot(df_power, aes(x = x, y = y_smooth, color = group)) +
  geom_line() +
  geom_point() +
  labs(
    x     = "Divergence time",
    y     = "Power",
    title = paste0("Power as divergence time varies (rolling mean, w=", w, ")"),
    color = "Legend Title"
  )
```

```{r compare mean power}
colSums(powers) / num_reps
```


## 2.4. Number of CSs

```{r num CSs, fig.width=8, fig.height=6}

colnames(no_CSs) <- c("In-sample", "Out-of-sample", "Naive shrinkage", "WS", "LW")
...
ggplot(df_CSs, aes(x = x, y = y_smooth, color = group)) +
  geom_line() +
  geom_point() +
  labs(
    x     = "Divergence time",
    y     = "Number of CSs",
    title = paste0("Number of CSs as divergence time varies (rolling mean, w=", w, ")"),
    color = "Legend Title"
  )
```
